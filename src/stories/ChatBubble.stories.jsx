...
// Light-mode baseline tokens (dark-mode overrides come from the global toolbar decorator)
const LyraTokens = () => (
  <style>{`
    :root {
...
export default {
  ...
  decorators: [
    Story => (
      <div style={{ padding: 24, maxWidth: 480, fontFamily: "Inter, sans-serif" }}>
        <LyraTokens />
        <Story />
      </div>
    ),
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "Unified chat bubble with two variants — **user** (right-aligned, blue tint, copy on hover/focus) and **ai** (left-aligned, full action bar with thumbs up/down, regenerate, and copy).",
      },
    },
  },
  argTypes: {
    ...
    isLatest: {
      control: "boolean",
      description: "AI only — keeps the action bar permanently visible",
      if: { arg: "variant", eq: "ai" },
    },
    reasoning: {
      control: "text",
      description: "AI only — collapsible reasoning/thinking text shown above the answer",
      if: { arg: "variant", eq: "ai" },
    },
    content: {
      control: "text",
      description: "Message text (user) or structured content array (ai)",
    },
  },
};
...
export const AI = {
  ...
  args: {
    ...
    content: [
      ...
      { type: "list", items: [
        "Calls in queue: 12 (↑4 from 10 min ago)",
        "Longest wait: 6:48",
        "Agents available: 2 of 6",
        "SLA at risk: Billing queue – projected breach in ~8 minutes",
      ]},
